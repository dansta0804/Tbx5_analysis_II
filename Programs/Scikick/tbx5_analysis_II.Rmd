...
This part of the work describes the analysis that was performed to determine
whether the search of synteny blocks is a suitable method that is used to
evaluate homology between *Mus musculus* and *Danio rerio* genomes.

- The samples that were used in the analysis are described in the previous
    analysis and can be viewed [here](https://karklas.mif.vu.lt/~dast6577/KursinisProjektas/samples.html);
- The results of the Tbx5 transcription factor abundance in *Mus musculus*
    organism analysis can be found [here](https://karklas.mif.vu.lt/~dast6577/KursinisProjektas/tbx5_analysis_I.html).
...
```{r Synteny_Chr_MM_Plot, out.width="60%", out.height="60%", fig.align = "center"}
# nolint start
...
```{r Synteny_Chr_DR_Plot, out.width="60%", out.height="60%", fig.align = "center"}
# nolint start
...
the greatest number of fragments has been detected in the 5, 7, 9, 13 and 17
...
### Sample peaks percentage in synteny blocks
It can be seen in the horizontal barchart below that overall percentage of the
sample peaks in synteny blocks did not exceed 35%.
...
```{r Synteny_Percentage_Plot, out.width="60%", out.height="60%", fig.align = "center"}
# nolint start
plot2 <- ggplot(df_final, aes(fill = indic, y = as.numeric(value), x = names)) + 
    ...
    scale_fill_manual(values = c("#d37f02", "#f3bc47"),
                      labels = c("Pikų skaičius",
                                 "Pikai, patenkantys į sintenijos blokus")) +
...
```{r Peak_Annotations_MM, message=FALSE, warning=FALSE, include=FALSE}
# nolint start
...
A diagram below shows how many unique genes were annotated for each house mouse
heart cell sample belonging to the synteny blocks.

```{r Peak_Annotations_MM_Plot, out.width="60%", out.height="60%", fig.align = "center"}
# nolint start
...
plot3 <- ggplot(genes_samples, aes(x = Sample, y = as.numeric(Percentage))) +
    ...
    labs(title = "", x = "",
    y = expression(paste("Anotuotų pikų % ", italic("M. musculus")))) +
    geom_text(aes(label = paste0(round(as.numeric(Percentage), digit = 2),
                                 "%")), color = "#030101", size = 5,
              vjust = -1, fontface = 2) +
    coord_cartesian(ylim = c(0,
                             as.numeric(max(genes_samples$Percentage)) + 20)) +
...
A diagram below shows how many unique zebrafish genes overlap with genes that
were determined in house mouse genome and belong to synteny blocks.
...
### Tbx5 PWM matrix hits
The chart below shows Tbx5 transcription factor PWM matrix hit counts among
*Mus musculus* and *Danio rerio* genes that overlap and belong to synteny
blocks.

```{r All_Annotations_MM, message=FALSE, warning=FALSE, include=FALSE}
grl_mm <- GRangesList()
...
find_common_genes <- function(sample, name_title) {
    ...
    for (i in 1:synteny_block_n) {
        ...
        # Finding common genes:
        cmmn_g <- intersect(tolower(genes_mm[,"gene_symbol"]),
        ...
        if (length(rownames(genes_dr)) != 0 & length(rownames(genes_mm)) != 0 &
            ...
            cg_data_dr <- genes_dr[genes_dr$"gene_symbol" %in% cmmn_g,
            ...
            cg_data_mm <- genes_mm[tolower(genes_mm$"gene_symbol") %in% cmmn_g,
            ...
            genes <- rbind(cg_data_dr, cg_data_mm[, c("seqnames", "start",
                                                      "end", "gene_symbol",
                                                      "organism")])
...
```{r PWM_Matches_All_Samples, out.width="60%", out.height="60%", echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center"}
# nolint start
...
### Conclusion
The results of the analysis indicated that the search of synteny blocks is not
the best way to find homologous regions between *Mus musculus* and *Danio rerio*
organisms, because the percentage of *Mus musculus* peaks that belong to synteny
block intervals is lower than 35%. It means that by analyzing ChIP-seq data
and determining peaks that belong to synteny blocks some of the homologous
regions are not recognized.