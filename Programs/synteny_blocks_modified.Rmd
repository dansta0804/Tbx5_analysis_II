...
plot0 <- ggplot(chr_syn, aes(x = Chr, y = Freq)) + 
    ...
    theme(panel.background = element_rect(fill = "#eeeef1",
          ...
          panel.grid.minor.x = element_line(colour = "#cab5b5", size = 0.2,
                                            linetype = "longdash"),
          axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, ize = 11,
...
plot3 <- ggplot(genes_samples, aes(x = Sample, y = as.numeric(Percentage))) +
    ...
    geom_text(aes(label = round(as.numeric(Percentage), digits = 2)),
...
png(file = paste0(FIGURES, "Unique_genes_MM.png"), width = 700)
plot3
dev.off()
plot3
# nolint end
...
df_mm <- df_mm[, c("seqnames", "start", "end", "width", "strand", "annotation",
                   "gene_symbol")]
                   
intersecting_genes <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(intersecting_genes) <- c("Gene", "Chr", "Start", "End", "Organism")
...
# belong_block[nrow(belong_block) + 1, ] <- c(name_title, count,
#                                             (count / synteny_block_n) * 100)
# }

# write.table(belong_block, file = paste0(INTER_FILES, "/Belong_to_block.txt"),
...
gene_hits_mm <- as.data.frame(gene_hits_mm[, list(Hits = sum(as.numeric(Hits)),
                            Length = sum(as.numeric(Length))), by = 'Gene'])
