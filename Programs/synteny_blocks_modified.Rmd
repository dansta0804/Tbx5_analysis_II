...
a <- as.data.frame(ranges(head(grl_reduced[[1]])))
a
...
gene_seq_mm <- getSeq(BSgenome.Mmusculus.UCSC.mm10, mm_gr)
names(gene_seq_mm) <- mm_gr$gene_symbol
...
for (gene in 1:length(gene_seq_dr)) {
    hits <- countPWM(as.matrix(mpwm), as.character(gene_seq_dr[gene]),
                     min.score = "75%")
    # matchPWM(as.matrix(mpwm), as.character(seq_dataset[[gene]]]),
            #   min.score = "75%")
    row = c(names(gene_seq_dr[gene]), hits,
            nchar(as.character(gene_seq_dr[gene])))
...
```{r}
# nolint start
plot3 <- ggplot(genes_synteny, aes(x = Sample, y = as.numeric(Gene_count))) +
    geom_bar(stat = "identity", fill = "#cc8b12", width = .6,
             color = "#8d5c00") +
    labs(title = "", x = "", y = "Anotuotų pikų skaičius M. Musculus") +
    geom_text(aes(label = Gene_count), color = "#030101", size = 5,
              just = -1, fontface = 2) +
    theme(axis.text = element_text(size = 10, colour = "black"),
          axis.text.x = element_text(angle = 45, hjust = 1, size = 12,
                                     vjust = 1, face = "bold"),
...
```{r Danio_rerio_sequences, echo=FALSE, message=FALSE, warning=FALSE}
# nolint start
seq_dataset_dr <- c()

# Creating a sequence dataset that stores Danio rerio gene sequences:
for (object in 1:length(grl_synteny)) {
    common_genes <- subsetByOverlaps(dr_genes[[object]], syn_grange_dr)
    gene_sequences <- getSeq(BSgenome.Drerio.UCSC.danRer11, common_genes)
    names(gene_sequences) <- common_genes$gene_symbol

    # Creating a sequence set:
    seq_dataset_dr <- c(seq_dataset_dr, gene_sequences)
    names(seq_dataset_dr)[object] <- names(grl_synteny)[object]
}
# nolint end
```

Tikrinama, kiek yra galimų atitikimų PWM.

```{r Danio_rerio_PWM, echo=FALSE, message=FALSE, warning=FALSE}
# nolint start
# Loading a Tbx5 PWM matrix:
mpwm <- read.table(paste0(INTER_FILES, "TBX5_MOUSE.H11MO.0.D.pwm"))
mpwm <- t(mpwm)
rownames(mpwm) <- c("A", "C", "G", "T")
...
#           Atrinkti tik tuos grafikus, kuriuose gaunamas geriausias
#           Scatterplot grafikas.
# nolint end
```

Čia išgaunamos Mus musculus genų sekos, kurios priklauso sintenijos blokui.

```{r Mus_musculus_genes, echo=FALSE, message=FALSE, warning=FALSE}
# nolint start
seq_dataset_mm <- c()

# Getting positions of Mus musculus genes:
...
select_genes <- tolower(gene_ranges$gene_symbol) %in%
                    tolower(grl_synteny[[6]]$gene_symbol)
...
seq_dataset_mm <- c(seq_dataset_mm, gene_sequences)
names(seq_dataset_mm)[6] <- names(grl_synteny)[6]

# Writing the results into a file:
write.table(seq_dataset_mm, file = paste0(RESULTS, "/seq_dataset_mm.txt"),
            sep = "\t", quote = FALSE, row.names = FALSE)

# nolint end
```

Čia tikrinama Mus musculus organizme, kiek yra pozicijų, kuriose gali
jungtis Tbx5 transkripcijos faktorius.

```{r Mus_musculus_PWM, echo=FALSE, message=FALSE, warning=FALSE}
# nolint start
gene_hits_mm <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(gene_hits_mm) <- c("Gene", "Hits", "Length")

# Determining the number of Tbx5 binding hits:
for (gene in 1:length(seq_dataset_mm[[1]])) {
    hits <- countPWM(as.matrix(mpwm), as.character(seq_dataset_mm[[1]][gene]),
                     min.score = "75%")
    # matchPWM(as.matrix(mpwm), as.character(seq_dataset[[gene]]]),
    #           min.score = "75%")
    row = c(names(seq_dataset_mm[[1]][gene]), hits,
            length(seq_dataset_mm[[1]][[gene]]))
    gene_hits_mm[nrow(gene_hits_mm) + 1, ] <- row
}

# Writing the results into a file:
write.table(gene_hits_mm, file = paste0(RESULTS, "/Mm_Hits.txt"), sep = "\t",
           quote = FALSE, row.names = FALSE)

scatterplot(as.numeric(Hits) ~ as.numeric(Length) , data = gene_hits_mm,
            xlab = "Weight of Car", ylab = "Miles Per Gallon",
            main = "Enhanced Scatter Plot")
# nolint end
```

```{r Compare_MM_DR, echo=FALSE, message=FALSE, warning=FALSE}
# nolint start
# Determining common genes:
intersects <- intersect(gene_hits_dr$Gene,tolower(gene_hits_mm$Gene))

data_about_intersects <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(data_about_intersects) <- c("Gene", "Hits_MM", "Length_MM",
                                     "Hits_DR", "Length_DR")

# Modifying the data and extracting needed gene data:
DR <- na.omit(gene_hits_dr[gene_hits_dr$Gene %in% intersects, ])
MM <- na.omit(gene_hits_mm[tolower(gene_hits_mm$Gene) %in% intersects, ])

# Ordering the data by Gene name:
DR <- DR[order(DR$Gene), ]
MM <- MM[order(MM$Gene), ]

MM$Gene <- tolower(MM$Gene)
na.omit(MM[order(MM$Gene), ])

# Combining Mus musculus and Danio rerio dataframes to compare the data:
merged_MM_DR <- merge(MM, DR, by = "Gene")

DR[DR$Gene == "acaa2",]
MM[MM$Gene == "acaa2",]

gene_hits_dr
a <- names(seq_dataset_mm[[1]]) == "Umad1"
as.character(seq_dataset_mm[[1]]["Acaa2"])
seq_dataset_dr[[1]]["acaa2"]


hits <- countPWM(as.matrix(mpwm), as.character(seq_dataset_mm[[1]]["Acaa2"]),
                     min.score = "75%")

hits2 <- countPWM(as.matrix(mpwm), as.character(seq_dataset_dr[[1]]["acaa2"]),
                     min.score = "75%")
matchPWM(as.matrix(mpwm), as.character(seq_dataset_dr[[1]]["acaa2"]),
            min.score = "75%")
