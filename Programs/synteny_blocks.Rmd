...
png(file = paste0(FIGURES, "Synteny_blocks_chromosomes.png"), width = 700)
plot0
dev.off()
plot0
# nolint end
...
plot1 <- ggplot(df_final, aes(fill = indicator, y = as.numeric(value),
                          x = names)) + 
...
```{r Peak_Annotations_Visuals, echo=FALSE, message=FALSE, warning=FALSE}
# nolint start
...
plot2 <- ggplot(annot_peaks, aes(x = sample_names, y = annotated_genes)) +
...
plot3 <- ggplot(annot_peaks, aes(x = sample_names, y = intersect_genes)) +
...
```{r}
grl <- lapply(grl, FUN = function(x) seqlengths(x) <- seqlengths(x) - 10001)

peakAnnoList <- lapply(grl, annotatePeak, TxDb=mm_known_genes,
