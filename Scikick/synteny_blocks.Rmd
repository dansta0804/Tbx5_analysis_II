...
# nolint start
synteny_blocks <- read.csv(paste0(PROJECT, "synteny_blocks.csv"), skip = 1)
samples <- read.csv(file = paste0(PROJECT, "sample_key.csv"))
...
bbfiles <- list.files(path = INPUTS, "*bb")
grl <- GRangesList()
...
for (i in 1:length(bbfiles)) {
    grl[[i]] <- import(paste0(INPUTS, bbfiles[i])) %>%
